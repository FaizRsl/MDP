...
from settings import API_IP, API_IP1, API_PORT
...
class RaspberryPi:
    ...
    def recv_stm(self) -> None:
        ...
        while True:
            ...
            potato = message.endswith("ACK|")
            self.logger.debug(f"{potato}")
            self.rs_flag = True
            if message.endswith("ACK|") and not message.startswith("ACK"):
                if self.rs_flag == False:
                    self.rs_flag = True
                    self.logger.debug("ACK for RS00 from STM32 received.")
                    continue
                try:
                    self.logger.debug("it runs here")
    ...
    def command_follower(self) -> None:
        ...
        while True:
            ...
            #self.logger.debug(f"current command: {command}")
            ...
            self.logger.debug("lock acquired for next instruction")
            self.logger.debug(f"Current instruction: {command}")
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        ...
        retry_count = 0
        #extn = ".jpg"
        #rpistr = "libcamera-still -e " + extn + " -n -t 500 -o " + filename
        ...
        self.logger.debug("it runs here after taking picture")
        self.movement_lock.release()
        try:
            self.retrylock.release()
        except:
    ...
    def request_algo(self, data, robot_x=1, robot_y=1, robot_dir=0, retrying=False):
        ...
        url = f"http://{API_IP1}:{API_PORT}/path"
